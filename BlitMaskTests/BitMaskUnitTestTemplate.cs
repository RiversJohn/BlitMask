...
namespace BlitMaskTests
{
    ...
    using BlitMaskExtensions = BlitMaskExtensions64;

    [TestClass]
    public class BitMaskUnitTestTemplate
    {
        [TestMethod]
        public void HasFlag()
        {
            ...
            Assert.IsTrue(bitMask.HasFlag(testValue), $"{nameof(BlitMaskExtensions.HasFlag)} failed unit test.");
        }
        ...
        public void HasAllFlags()
        {
            ...
            /// This presumes that cast from <see cref="BitMask"/> to <see cref="uint"/> functions correctly
            Assert.IsTrue(bitMask.HasAllFlags(expectedValue), $"{nameof(BlitMaskExtensions.HasFlag)} failed unit test.");
        }
        ...
        public void HasAnyFlag()
        {
            ...
            BitMask incorrectFlag = new BitMask(2);

            /// This presumes that cast from <see cref="BitMask"/> to <see cref="uint"/> functions correctly
            Assert.IsTrue(bitMask.HasAnyFlag(anyFlag) && !bitMask.HasAnyFlag(incorrectFlag), $"{nameof(BlitMaskExtensions.HasFlag)} failed unit test.");
        }
        ...
        {
            Assert.IsTrue(BlittableTypeTest.IsBlittableType(typeof(BitMask)));
        }
    }
}
